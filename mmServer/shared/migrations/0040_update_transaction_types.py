...
class Migration(DataMigration):
    """ A manual data migration to update the transaction types.

        We convert from the "S" and "R" (system charge and recipient charge)
        types to the new generic "C" (charge) type.  When going backwards, we
        check the account being credited to see if the charge was a system
        charge or a recipient charge.
    """
    def forwards(self, orm):
        """ Forward migration for updating the transaction types.
        """
    ...
    def backwards(self, orm):
        """ Backwards migration for updating the transaction types.
        """
        for transaction in orm.Transaction.objects.all():
            if transaction.type == "C":
                if transaction.credit_account.type == "M":
                    transaction.type = "S"
                    transaction.save()
                elif transaction.credit_account.type == "U":
                    transaction.type = "R"
                    transaction.save()
